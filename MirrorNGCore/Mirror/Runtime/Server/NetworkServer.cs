...
namespace Mirror.Runtime.Server
{
...
#endif
    {
...
#else
            if (logger.LogEnabled()) logger.Log("NetworkServer Created version " + Assembly.GetAssembly(typeof(NetworkServer))?.GetName().Version);
#endif
        ...
        internal void SendToObservers<T>(NetworkIdentity identity, T msg, bool includeOwner = true, byte channelId = (byte) Channels.Reliable)
        {
        ...
        public void SendToAll<T>(T msg, byte channelId = (byte) Channels.Reliable)
        {
        ...
        /// <param name="identity"></param>
        /// <param name="msg"></param>
        public void SendToClientOfPlayer<T>(NetworkIdentity identity, T msg, byte channelId = (byte) Channels.Reliable)
        {
            if (identity != null)
            {
